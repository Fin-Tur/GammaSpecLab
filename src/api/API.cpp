...
int estimate_baseline_als(const double* counts, std::size_t count_size, double lambda, double p, int max_iterations, double* baseline) {
    if (!counts || !baseline || count_size == 0 || max_iterations <= 0) return -1;

    try {
        std::vector<double> input(counts, counts + count_size);
        std::vector<double> estimate = ALS_BASELINE::estimateBackgroundUsingALS(input, lambda, p, max_iterations);
        if (estimate.size() != count_size) return -2;
